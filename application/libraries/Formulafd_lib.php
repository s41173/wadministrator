...
class Formulafd_lib {
    ...
    function calculate($type=null,$width=0,$height=0,$pid=0,$heightkm=0){
        ...
        case "LIDAH KUSEN": $res = $this->c_lidah_kusen($width, $height, $heightkm); break;
        case "LIDAH DAUN": $res = $this->c_lidah_daun($width, $height, $heightkm); break;
        case "ALUR KUNCI": $res = $this->c_slot_lock($width, $height, $heightkm); break;
        } return $res;
    }
    ...
    function c_lidah_kusen($width=0,$height=0,$heightkm=0)
    {
        if ($this->daunhidup == 0){ return floatval($height);}else{ return floatval(0); }
    }
    
    function c_lidah_daun($width=0,$height=0,$heightkm=0)
    {
        if ($this->daunhidup == 0){ return floatval($height);}else{ return floatval(2*$height); }
    }
    
    function c_slot_lock($width=0,$height=0,$heightkm=0){
        $res1 = floatval($this->daun-1);
        return floatval($res1*$height);
    }
    ...
    {
        $res1 = floatval(2*$width);
        $res2 = floatval($this->daun*2*$height);
        return floatval($res1+$res2);
    }
