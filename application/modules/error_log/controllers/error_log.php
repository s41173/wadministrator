...
class Error_log extends MX_Controller
{
    ...
    {
        if(!$search){ $result = $this->Log_model->get_last($this->modul['limit'])->result(); }
	 ...
	 {
	   $output[] = array ($res->id, $this->com->get_name($res->modul), $res->message, $res->created);
	 } 
	...
	// ---------------------------------------- //
        ...
        $this->table->set_heading('#','No', 'Modul', 'Error', 'Time');
