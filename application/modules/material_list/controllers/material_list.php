...
class Material_list extends MX_Controller
{
    function __construct()
    {
        parent::__construct();
        
        $this->load->model('Material_list_model', 'model', TRUE);
	...
	$data['form_action'] = site_url($this->title.'/add_process');
        ...
        $data['link'] = array('link_back' => anchor('material','Back', array('class' => 'btn btn-danger')));
	// ---------------------------------------- //
    ...
    private function cek_relation($id)
    {
        ...
        $product = $material->cek_relation($id, 'material_list');
	...
	// Form validation
        ...
        {
            $model = array('name' => strtoupper($this->input->post('tname')), 'created' => date('Y-m-d H:i:s'));
	...
	// Form validation
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            $model = array('name' => strtoupper($this->input->post('tname_update')));
